...
def func_create_internal_group(client, **kwargs):
    ...
    try:
        ...
        client.secrets.identity.create_or_update_group_by_name(
            name=kwargs.get('int_grp_name'),  #'{}-{}-{}'.format(project_id, project_name, policy_kind),
            group_type='internal',
            policies=kwargs.get('int_policies'),
        ...
        print(f"\n{kwargs.get('message')}")
        print(f"{kwargs.get('int_grp_name')}")  #'{}-{}-{}'.format(project_id, project_name, policy_kind))
        print(f"policies: {kwargs.get('int_policies')}")
...
def func_internal_groups(client, **kwargs):
    ...
    adm_ext_member_ids =[value for key,value in kwargs.get('external_groups').get('adm_ext_groups').items()]
    adm_int_grp_name   =f"{kwargs.get('prefix')}_{kwargs.get('L2_APMID')}_adm"
    d_values ={ 'int_grp_name': adm_int_grp_name, 'int_policies': adm_int_grp_policies,
                'external_groups': adm_ext_member_ids, 'message': 'Admin internal group added!',
                'metadata': metadata
                }
    ...
    rw_ext_member_ids =[value for key,value in kwargs.get('external_groups').get('rw_ext_groups').items()]
    rw_int_grp_name =f"{kwargs.get('prefix')}_{kwargs.get('L2_APMID')}_rw"
    d_values ={ 'int_grp_name': rw_int_grp_name, 'int_policies': rw_int_grp_policies,
                'external_groups': rw_ext_member_ids, 'message': 'Read/Write internal group added!',
                'metadata': metadata
                }
    ...
    ro_ext_member_ids =[value for key,value in kwargs.get('external_groups').get('ro_ext_groups').items()]
    ro_int_grp_name =f"{kwargs.get('prefix')}_{kwargs.get('L2_APMID')}_ro"
    d_values ={ 'int_grp_name': ro_int_grp_name, 'int_policies': ro_int_grp_policies,
                'external_groups': ro_ext_member_ids, 'message': 'Read Only internal group added!',
                'metadata': metadata
                }
