...
def func_vault_only_email_templating(**kwargs):
    ...
    try:    
        ...
        with open(currentdir + '/' + kwargs.get('email_template'), 'r') as email:
            content =email.read()
            t =Template(content)
            html =t.render(ENV=kwargs.get('environment'), L2_APM_ID=kwargs.get('L2_APMID'), L2_Deployable_Unit_Name=kwargs.get('l2_asset_name'), L3_list=kwargs.get('l3_asset_list'), 
