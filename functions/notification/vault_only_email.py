...
def func_vault_only_email_templating(**kwargs):

    subject = (
        kwargs.get("email_subject")
        .replace("{{ L2_APM_ID }}", kwargs.get("L2_APMID"))
        .replace("{{ L2_Deployable_Unit_Name }}", kwargs.get("l2_asset_name"))
        .replace("{{ ENV }}", kwargs.get("environment"))
    )
    print(f"\nReceiver: {kwargs.get('l2_owner')}")
    print(f"Vault Only Subject: {subject}")
    try:
        ...
        with open(currentdir + "/" + kwargs.get("email_template"), "r") as email:
            ...
            html = t.render(
                ...
                L2_APM_ID=kwargs.get("L2_APMID"),
                L2_Deployable_Unit_Name=kwargs.get("l2_asset_name"),
                L3_list=kwargs.get("l3_asset_list"),
                tev_env_L2_APMDU_ID_adm=kwargs.get("ad_group_adm"),
                tev_env_L2_APMDU_ID_rw=kwargs.get("ad_group_rw"),
                tev_env_L2_APMDU_ID_ro=kwargs.get("ad_group_ro"),
            )
        ...
        if kwargs.get("sending_status"):
            server.sendmail(
                kwargs.get("sender"), kwargs.get("l2_owner"), message.as_string()
            )
        server.quit()
