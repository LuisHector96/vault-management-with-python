...
def func_security_group_request(**kwargs):
    ...
    subject =kwargs.get('email_subject').replace('<group_name>', kwargs.get('group_name'))
    ...
    try:    
        ...
        with open(currentdir + '/' + kwargs.get('email_template'), 'r') as email:
            content =email.read()
            t =Template(content)
            html =t.render(l2_owner=kwargs.get('l2_owner_ntid') , env=kwargs.get('environment'), group_name=kwargs.get('group_name'), L2_CSDMID=kwargs.get('L2_APMID'), role=kwargs.get('role'), L2_application_name=kwargs.get('l2_asset_name'))
        ...
        if kwargs.get('sending_status'):
            server.sendmail(kwargs.get('sender'), kwargs.get('receiver'), message.as_string())
        server.quit()
