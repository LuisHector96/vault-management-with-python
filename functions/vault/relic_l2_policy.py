...
def func_relic_l2_policies(client, **kwargs):
    ...
    return_data[
        "l2_policy_adm_name"
    ] = f"{kwargs.get('prefix')}-{kwargs.get('L2_APMID').lower()}-shared-adm"
    return_data["l2_policy_adm"] = kwargs.get("admin_shared_policy").replace(
        "L2_APMID", kwargs.get("L2_APMID")
    )

    return_data[
        "l2_policy_rw_name"
    ] = f"{kwargs.get('prefix')}-{kwargs.get('L2_APMID').lower()}-shared-rw"
    return_data["l2_policy_rw"] = kwargs.get("read_write_shared_policy").replace(
        "L2_APMID", kwargs.get("L2_APMID")
    )

    return_data[
        "L2_policy_ro_name"
    ] = f"{kwargs.get('prefix')}-{kwargs.get('L2_APMID').lower()}-shared-ro"
    return_data["L2_policy_ro"] = kwargs.get("read_only_shared_policy").replace(
        "L2_APMID", kwargs.get("L2_APMID")
    )

    d_values = {
        "policy_name": return_data.get("l2_policy_adm_name"),
        "policy": return_data.get("l2_policy_adm"),
        "role": "L2 Admin",
    }
    func_l2_policy(client, **d_values)
    d_values = {
        "policy_name": return_data.get("l2_policy_rw_name"),
        "policy": return_data.get("l2_policy_rw"),
        "role": "L2 Read/Write",
    }
    func_l2_policy(client, **d_values)
    d_values = {
        "policy_name": return_data.get("L2_policy_ro_name"),
        "policy": return_data.get("L2_policy_ro"),
        "role": "L2 Read Only",
    }
    func_l2_policy(client, **d_values)
